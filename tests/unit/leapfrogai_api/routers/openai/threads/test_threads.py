...
from leapfrogai_api.routers.openai.threads import create_thread, retrieve_thread, modify_thread, delete_thread
...
async def test_modify_thread(mock_session):
    mock_session.table().select().execute.return_value = execute_response_format(mock_thread.model_dump())
    mock_session.table().update().execute.return_value = execute_response_format(mock_thread.model_dump())

    mock_request = ModifyThreadRequest(
        tool_resources=None,
        metadata = dict(new_data="mock-data")
    )
...
async def test_delete_thread(mock_session):
    mock_session.table().delete().execute.return_value = execute_response_format(mock_thread.model_dump())
