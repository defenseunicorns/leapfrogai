...
setup-llama-cpp-python-deps:  ## Download the wheels for the optional 'llama' dependencies
	-rm packages/llama-cpp-python/build/*.whl
	python -m pip wheel ".[llama-cpp-python]" -w packages/llama-cpp-python/build

build-llama-cpp-python: local-registry setup-llama-cpp-python-deps ## Build the llama (cpu) container and Zarf package
	## Build the image (and tag it for the local registry)
